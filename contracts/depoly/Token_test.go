...
func TestENS(t *testing.T) {
	contractBackend := backends.NewSimulatedBackend(types.GenesisAlloc{
		addr:     {Balance: big.NewInt(1000000000000)},
	...
	fmt.Println("Token name:", name, " ", ensAddr.String())
	...
	contractBackend.Commit()
	fmt.Println("testAddr", testAddr.String(), " addr ", addr.String())
	...
	fmt.Println("begin balance", balance)
	...
	balance, _ = contractBackend.BalanceAt(context.Background(), testAddr, nil)
	fmt.Println("end balance", balance)
}
