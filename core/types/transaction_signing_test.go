...
func TestEIP155SigningTx(t *testing.T) {
	...
	signer := NewTIP1Signer(new(big.Int).SetUint64(1000))
	...
	fmt.Println("i", weiToEth(i), " fee ", weiToEth(new(big.Int).Mul(gas, new(big.Int).SetUint64(gasLimit))), "addr", addr)

	tx := NewTransaction_Payment(1, common.HexToAddress("0x92b28647ae1f3264661f72fb2eb9625a89d88a31"), big.NewInt(1811266580600000000), new(big.Int).SetUint64(0), gasLimit, gas, nil, addr)
	tx, err = SignTx(tx, signer, key)
	if err != nil {
		t.Fatal(err)
	}
	tx, err = SignTx_Payment(tx, signer, key)
	if err != nil {
		t.Fatal(err)
	}

	from, err := Payer(signer, tx)
	if err != nil {
		t.Fatal(err)
	}
	if from != addr {
...
func TestEIP155Signing(t *testing.T) {
	...
	signer := NewTIP1Signer(big.NewInt(18))
...
func TestEIP155SigningRlp(t *testing.T) {
	...
	fmt.Println("from", from, "addr", addr)
	if from != addr {
		t.Errorf("exected from and address to be equal. Got %x want %x", from, addr)
	}
}
...
func TestEIP155SigningVitalik(t *testing.T) {
	...
	for i, test := range []struct {
		...
		signer := NewTIP1Signer(big.NewInt(1))
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTx(tx, NewTIP1Signer(big.NewInt(1)), key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(NewTIP1Signer(big.NewInt(2)), tx)
	if err != ErrInvalidChainId {
		t.Error("expected error:", ErrInvalidChainId)
	}

	_, err = Sender(NewTIP1Signer(big.NewInt(1)), tx)
	if err != nil {
